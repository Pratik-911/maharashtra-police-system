...
class DutyModel extends Model
{
    ...
    {
        $builder = $this->select('duties.*, points.name as point_name, points.latitude as point_latitude, points.longitude as point_longitude, points.radius as point_radius')
                        ->join('points', 'points.point_id = duties.point_id')
                        ->join('duty_officers', 'duty_officers.duty_id = duties.duty_id')
                        ->where('duty_officers.officer_id', $officer_id)
                        ->orderBy('duties.date', 'DESC')
    ...
    public function getDutyOfficers($duty_id)
    {
        $builder = $this->db->table('duty_officers')
                           ->select('officers.*, duty_officers.duty_id')
    ...
    public function getActiveDutiesForStation($stationCode)
    {
        $currentDate = date('Y-m-d');
        $currentTime = date('H:i:s');
        ...
        return $this->select('duties.*, points.name as point_name')
                    ->join('points', 'points.point_id = duties.point_id')
                    ->where('points.police_station_id', $stationCode)
                    ->where('duties.date', $currentDate)
                    ->where('duties.start_time <=', $currentTime)
                    ->where('duties.end_time >=', $currentTime)
                    ->findAll();
    ...
    public function getRecentDutiesForStation($stationCode, $limit = 10)
    {
        return $this->select('duties.*, points.name as point_name')
                    ->join('points', 'points.point_id = duties.point_id')
                    ->where('points.police_station_id', $stationCode)
    ...
    public function getDutiesWithPointsForStation($stationCode)
    {
        return $this->select('duties.*, points.name as point_name, points.police_station_id')
                    ->join('points', 'points.point_id = duties.point_id')
                    ->where('points.police_station_id', $stationCode)
                    ->orderBy('duties.date', 'DESC')
                    ->orderBy('duties.start_time', 'DESC')
